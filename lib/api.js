...
api.interceptors.response.use(
  ...
  async (error) => {
      ...
      if (refreshToken) {
        try {
          const refreshResponse = await axios.post(
            backendUrl ? `${backendUrl}/api/v1/auth/refresh-token` : '/api/v1/auth/refresh-token',
            { refreshToken }
          );
    ...
    // Normalize backend error message for frontend pages expecting err.response.data.error
...
export const authAPI = {
  ...
  googleAuthComplete: (payload) => api.post(`${endpoints.auth}/google/complete`, payload),
