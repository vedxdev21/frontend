...
import { ArrowLeft, ArrowRight, Check, Upload, X, MapPin } from 'lucide-react';
...
const mapDBToUI = (dbVal, map, fallback) => {
  const entry = Object.entries(map).find(([ui, db]) => db === dbVal);
    ...
    const fetchProperty = async () => {
      try {
        ...
        setForm({
          ...
          propertyType: mapDBToUI(p.propertyType, PROPERTY_TYPE_MAP, '2BHK'),
          category: mapDBToUI(p.category, CATEGORY_MAP, 'Residential'),
          availableFor: mapDBToUI(p.availableFor, AVAILABLE_FOR_MAP, 'Anyone'),
          dependency: p.dependency || 'Independent',
          rent: p.rent?.toString() || '',
          deposit: p.deposit?.toString() || '',
          negotiable: mapDBToUI(p.negotiable, NEGOTIABLE_MAP, 'Fixed'),
          maintenanceExtra: p.maintenanceExtra || false,
          maintenanceAmount: p.maintenanceAmount?.toString() || '',
          minimumStay: mapDBToUI(p.minimumStay, MINIMUM_STAY_MAP, '6 Months'),
          availableFrom: p.availableFrom ? new Date(p.availableFrom).toISOString().split('T')[0] : '',
          furnishing: mapDBToUI(p.furnishing, FURNISHING_MAP, 'Semi Furnished'),
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          <h1 className="text-2xl font-extrabold text-gray-900 mb-2">Edit Your Property</h1>
          <p className="text-sm text-gray-500 mb-6">Update the details of your property listing.</p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="flex justify-between mt-6 pt-4 border-t border-gray-100">
              ...
              {step < 4 ? (
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleSubmit(form.status)} disabled={loading}
                    className="btn-primary flex items-center gap-1.5">
                    {loading ? <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><Check className="w-4 h-4" /> Save Changes</>}
                  </button>
                </div>
              )}
            </div>
